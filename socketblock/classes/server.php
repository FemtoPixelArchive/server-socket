...
class Server_Socket {
	...
	public function sendMessage($socket, $resp) {
	...
	public function setClient($tel, $socket = NULL) {
		if ($socket) {
			if ($this->getClient($tel)) {
				...
				$this->_clients[$tel] = $socket;
			}
		} else {
			$this->log("Client $tel disconnected");
			if ($this->getClient($tel)) {
				socket_close($this->_clients[$tel]);
			}
	...
	public function getClient($tel) {
		return ((isset($this->_clients[$tel]) && is_resource($this->_clients[$tel])) ? $this->_clients[$tel] : NULL);
	}
